...
void input_device_init(VirtioDevice *device) {
    ...
    infof("input_device_init: prod_id = 0x%x\n", prod_id);
    ...
    infof("Input device init done for device at %p\n", device->pcidev->ecam_header);
}
...
void get_input_device_config(VirtioDevice *device, uint8_t select, uint8_t subsel, uint8_t size) {
    ...
    infof("set_input_device_config: select = 0x%x\n", config->select);
    config->subsel = subsel;
    infof("set_input_device_config: subsel = 0x%x\n", config->subsel);
    infof("%.128s\n", config->string);
}
...
void input_device_push_event(InputDevice *input_dev, VirtioInputEvent event) {
    if (input_dev == NULL) {
        ...
        warnf("input_device_push_event: Invalid input event: type = 0x%x, code = 0x%x, value = 0x%x\n", event.type, event.code, event.value);
        return;
    } else {
        infof("Pushing event to %s\n", input_dev->viodev->name);
    }
    ...
    if (input_dev->buffer_count < INPUT_EVENT_BUFFER_SIZE) {
        ...
        infof("input_device_isr: Input event received: type = 0x%x, code = 0x%x, value = 0x%x\n", event.type, event.code, event.value);
    } else {
...
void handle_input(InputDevice *device) {
    ...
    infof("input_device_isr: Starting ISR for %.60s\n", viodev->name);
