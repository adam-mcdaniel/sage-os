...
# 5-Nov-2023
- `amcdan23`: Expanded the block driver to work with any block device. Added functionality for: mounting devices on the VFS, opening files on arbitrary mounted devices on the VFS (multiple open files on multiple drives simultaneously), reading/writing files on VFS, and automatically populating mounted drives in `/dev` folder on the base drive \#0 at `/`. Wrote `vfs_read`, `vfs_write`, `vfs_init`, `vfs_open`, `vfs_close`, `vfs_mount`, and rewrote all the block driver functions with devices as parameters. Added a PCI function for getting the Nth device of a given type.
